...
import {
  GetProductRes,
  getProductRes2Product,
} from '../api-req-res/getProductRes';
...
export class ProductDAOService {
  ...
  getProducts(): Promise<Product[]> {
    ...
    return new Promise((resolve) => {
      ...
      this.http
        .get<GetProductRes[]>(baseUrl + '/api/product')
        .subscribe((res) => {
  ...
  addNewProduct(jwt: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.http
        .post(baseUrl + '/api/product', addProductReq, {
  ...
  deleteProduct(id: number, jwt: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.http
        .delete(baseUrl + '/api/product/' + id.toString(), {
          headers: headers_object,
        })
        .subscribe(
          (res) => {
